...
def value_iteration(epsilon):
    ...
    rlt_path = os.path.join('part1/result','value iteration','vi - '+str(epsilon))
    try:
        os.makedirs(rlt_path)
    except os.error:
        pass

    rlt_f = os.path.join(rlt_path,'vi - '+str(epsilon) +'.csv' )
    if os.path.exists(rlt_f):
        os.remove(rlt_f)

    log_f = os.path.join(rlt_path,'log_vi - '+str(epsilon) +'.txt' )
    ...
    plot_utility_iteration(1,"v",epsilon)
